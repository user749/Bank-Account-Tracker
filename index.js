...
//if we have something in local storage parse it, otherwise create an empty list
...
function addTransaction(source, amount){
    ...
    //storing our transaction object in local storage
...
function deleteTransaction(id){
    transactions = transactions.filter(transaction => {
        return transaction.id !== id; //return list without our element, leave only the one that we are going to delete
    });
    localStorage.setItem("transactions", JSON.stringify(transactions));
}
